...
def run_eda(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    sns.boxplot(data=df[["churn", "monthly_premium"]], x="churn", y="monthly_premium")
    plt.title("Prime mensuelle par statut de churn")
    plt.tight_layout()
    plt.savefig(output_dir / "boxplot_premium_churn.png", dpi=180)
    plt.close()
